"""Sweep stop_delay_hours for the best hybrid combo (21% + snap176 top3)."""
...
async def main():
    ...
    print(f"{'delay_h':>7} {'PnL':>10} {'#':>3} {'W%':>5} {'Sharpe':>7}  {'Mar20':>14} {'Mar27':>14} {'Apr3':>14}  {'Open':>4}")
    print("-" * 100)

    for delay in [12, 24, 36, 48, 60, 72, 84, 96, 108, 120, 144, 168]:
        params = {
            "trend_lookback": 12, "sell_trend_pct": 0.02,
            "min_day": 0, "max_day": 999, "min_crash": 0.05, "trailing_stop": 1.0,
            "stop_delay_hours": delay, "max_hold_hours": 336,
        ...
        print(f"{delay:>7} {r['total_pnl']:>+10,} {r['total_trades']:>3} {r['win_rate']:>4.0%} {r['sharpe_ratio']:>7.3f}  {fmt_batch(bp,'03-20'):>14} {fmt_batch(bp,'03-27'):>14} {fmt_batch(bp,'04-03'):>14}  {open_count:>4}")

    # Show trades for interesting delay values
    print(f"\n{'='*120}")
    for delay in [48, 72, 96]:
        params = {
            "trend_lookback": 12, "sell_trend_pct": 0.02,
            "min_day": 0, "max_day": 999, "min_crash": 0.05, "trailing_stop": 1.0,
            "stop_delay_hours": delay, "max_hold_hours": 336,
        ...
        print(f"\n--- delay={delay}h | PnL: {r['total_pnl']:+,} | Win: {r['win_rate']:.0%} | Trades: {r['total_trades']} ---")
        for t in sorted(r["trades"], key=lambda t: t["buy_time"]):
