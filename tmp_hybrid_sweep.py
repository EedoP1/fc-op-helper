"""Hybrid: always buy 21%+ trend cards + at 176h snapshot buy top N remaining."""
...
class HybridPromoDipBuy(PromoDipBuyStrategy):
    """Always buy 21%+ trend. At snapshot_hour, also buy top N remaining by trend."""

    name = "hybrid_promo_dip"
    ...
    def on_tick_batch(
        ...
        # --- BUY LAYER 1: always buy 21%+ trend (original logic) ---
        strong_buys = []
        for ea_id, price in ticks:
            if ea_id not in self._created_at_map:
                continue
            ...
            if trend >= self.trend_pct:  # 0.21
                strong_buys.append((ea_id, price))

        # --- BUY LAYER 2: at snapshot_hour, buy top N remaining ---
        snapshot_buys = []
        for batch_key, batch_created in self._batch_created.items():
            ...
            # Already bought from strong signal
...
async def main():
    ...
    base_params = {
        ...
        "trend_pct": 0.21,  # strong signal threshold
    }
    ...
    orig = PromoDipBuyStrategy({**base_params, "trend_pct": 0.20})
    r = run_backtest(orig, price_data, budget, created_at_map=created_at_map)
    bp = batch_pnl(r["trades"])

    print(f"{'Config':<45} {'PnL':>10} {'#':>3} {'W%':>5} {'Sharpe':>7}  {'Mar20':>14} {'Mar27':>14} {'Apr3*':>14}")
    print("-" * 125)
    ...
    print(f"{'Original 20%':<45} {r['total_pnl']:>+10,} {r['total_trades']:>3} {r['win_rate']:>4.0%} {r['sharpe_ratio']:>7.3f}  {fmt_batch(bp,'03-20'):>14} {fmt_batch(bp,'03-27'):>14} {fmt_batch(bp,'04-03'):>14}")

    # Also run with 21% to see how many it drops
    orig21 = PromoDipBuyStrategy({**base_params, "trend_pct": 0.21})
    r = run_backtest(orig21, price_data, budget, created_at_map=created_at_map)
    bp = batch_pnl(r["trades"])
    print(f"{'Original 21% (strong only)':<45} {r['total_pnl']:>+10,} {r['total_trades']:>3} {r['win_rate']:>4.0%} {r['sharpe_ratio']:>7.3f}  {fmt_batch(bp,'03-20'):>14} {fmt_batch(bp,'03-27'):>14} {fmt_batch(bp,'04-03'):>14}")
    print()

    # Sweep: hybrid with snapshot_top_n
    for snap_n in [0, 1, 2, 3, 4, 5]:
        for floor in [0.0, 0.05]:
            params = {
                **base_params,
                "snapshot_top_n": snap_n,
                "snapshot_hour": 176,
                "snapshot_floor": floor,
            }
            strategy = HybridPromoDipBuy(params)
            r = run_backtest(strategy, price_data, budget, created_at_map=created_at_map)
            bp = batch_pnl(r["trades"])
            label = f"21%+strong + snap176 top{snap_n} floor{floor:.0%}"
            print(f"{label:<45} {r['total_pnl']:>+10,} {r['total_trades']:>3} {r['win_rate']:>4.0%} {r['sharpe_ratio']:>7.3f}  {fmt_batch(bp,'03-20'):>14} {fmt_batch(bp,'03-27'):>14} {fmt_batch(bp,'04-03'):>14}")

    # Show trades for the best ones
    print(f"\n{'='*120}")
    print("TRADE DETAILS")
    print(f"{'='*120}")
    ...
    for snap_n in [2, 3]:
        for floor in [0.0, 0.05]:
            params = {**base_params, "snapshot_top_n": snap_n, "snapshot_hour": 176, "snapshot_floor": floor}
            strategy = HybridPromoDipBuy(params)
            r = run_backtest(strategy, price_data, budget, created_at_map=created_at_map)
            bp = batch_pnl(r["trades"])

            label = f"21%+strong + snap176 top{snap_n} floor{floor:.0%}"
            print(f"\n--- {label} | PnL: {r['total_pnl']:+,} | Win: {r['win_rate']:.0%} ---")

            trades_sorted = sorted(r["trades"], key=lambda t: t["buy_time"])
            for t in trades_sorted:
                name = names.get(t["ea_id"], str(t["ea_id"]))
                created = created_at_map.get(t["ea_id"])
                batch = created.strftime("%m-%d") if created else "?"
                fs = " [OPEN]" if t["sell_time"][:10] == "2026-04-08" else ""
                print(f"  [{batch}] {name[:22]:<23} {t['qty']:>3}x  buy {t['buy_price']:>7,} @ {t['buy_time'][5:13]}  sell {t['sell_price']:>7,} @ {t['sell_time'][5:13]}  pnl {t['net_profit']:>+9,}{fs}")
