"""Floor buy turnover_v1 — v36 fast-cycle with 20 slots, smaller per-slot qty.

Iter 50 hypothesis: higher turnover is the most promising path to $2M. combo_v18
did only 33 trades for +$150k (~$4.5k net per trade). The $2M target requires
hundreds of trades or much larger per-trade PnL.

Strategy: copy v36 verbatim; bump max_positions 8→20 and shrink qty_small/med/large
(10/18/25 → 4/6/8) to keep $1.4M notional within $1M cash after sell recycling.
At 96h cycle, each slot turns ~5.5x over 22 days → ~110 trades.

All other params identical to v36 (floor_ceiling=13000, profit_target=0.08, etc).
"""
import logging
...
class FloorBuyTurnoverV1Strategy(Strategy):
    name = "floor_buy_turnover_v1"
    ...
    def __init__(self, params: dict):
        ...
        self.max_positions: int = params.get("max_positions", 20)
        self.min_age_days: int = params.get("min_age_days", 7)
        self.burn_in_h: int = params.get("burn_in_h", 72)
        self.qty_small: int = params.get("qty_small", 4)
        self.qty_medium: int = params.get("qty_medium", 6)
        self.qty_large: int = params.get("qty_large", 8)
