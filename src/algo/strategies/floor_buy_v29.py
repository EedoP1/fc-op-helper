"""Floor buy v29 — v21 + wider floor band (admit more candidates/week).

v21 diagnostic (iter 18):
  Organic +$502.8k / 93.8% win / 16 organic trades over 27 days.
  W14 org: $0 (signal-coverage gap)  W15: +$29k  W16: +$473k (+47.3%).
  4/6 bars — fails W14 (2a) and corr (-0.829).

Iter 18 finding: v21 issues only ~15 buys in 27 days, so W14 starves.
v28 (profit-cooldown) didn't help because profitable cards don't dip
back below $13k floor_ceiling quickly. Targeted fix: widen the band
to admit more floor-cohort candidates per week without loosening other
quality filters.

Changes vs v21 (4 params):
  floor_ceiling:    13000 → 15000
  floor_stable:     14500 → 17000
  week_max_ceiling: 18000 → 22000
  min_price:        10000 → 11000  (slight upward floor; skip worst)

Expected: ~1.5-2x trade count, W14 populated. Risk: wider band admits
lower-quality cards → win rate dips.
"""
import logging
...
class FloorBuyV29Strategy(Strategy):
    name = "floor_buy_v29"
    ...
    def __init__(self, params: dict):
        ...
        self.floor_ceiling: int = params.get("floor_ceiling", 15000)
        self.floor_stable: int = params.get("floor_stable", 17000)
