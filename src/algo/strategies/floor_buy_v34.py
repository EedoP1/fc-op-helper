"""Floor buy v34 — v19 with short hold + lower profit target.

v19 holds up to 240h with a 50% profit target. That means BUYs in W14
often don't close until W15/W16, starving bar 2a (W14 organic PnL).

v34 tightens the cycle:
  - max_hold_h: 240 → 96 (exits land within ~4 days of entry)
  - profit_target: 0.50 → 0.20 (take smaller wins sooner)

Everything else (band, gates, qty tiers, week guard) stays identical to
v19. Designed to live alongside v19 in combo_v14 as a short-hold
supplement — same candidates, faster cash cycle so buys AND sells
inside W14 both register there.
"""
import logging
...
class FloorBuyV34Strategy(Strategy):
    name = "floor_buy_v34"
