"""Floor buy v10 — v9 grid sweep.

v9 locked: +$62k / 59.5% win / corr -0.321. v10 sweeps the key knobs
around v9 to search for a PnL bump without losing win rate:
  - qty_large: 10, 12, 14   (scale conviction trades)
  - qty_small: 4, 5         (scale fallback trades)
  - profit_target: 0.25, 0.30  (earlier exit vs ride)
  - max_hold_h: 96, 120     (give winners more time)
  - week_max_ceiling: 16000, 18000  (tighter vs same filter)

Best combo becomes the locked champion for the final research doc.
"""
import logging
...
class FloorBuyV10Strategy(Strategy):
    name = "floor_buy_v10"
    ...
    def param_grid_hourly(self) -> list[dict]:
        ...
        grid = []
        for profit_target in [0.25, 0.30]:
            for max_hold in [96, 120]:
                for week_max in [16000, 18000]:
                    for qty_large in [10, 12, 14]:
                        for qty_small in [4, 5]:
                            grid.append({
                                **base,
                                "profit_target": profit_target,
                                "max_hold_h": max_hold,
                                "week_max_ceiling": week_max,
                                "qty_small": qty_small,
                                "qty_large": qty_large,
                            })
        return grid
