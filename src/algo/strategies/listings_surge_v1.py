"""Listings Surge v1 — inventory-shock entry via listing_count drop.

Genuinely new signal lane: uses the hourly avg listing_count data that's
loaded by the engine and passed via set_listing_counts() but which no
existing strategy actually consumes. Theory: when supply dries up faster
than new listings appear (a 30%+ drop in listing_count over 24h) but the
price hasn't yet spiked (<=3% rise same window), we're buying pre-spike
in an inventory-shock regime.

Entry (per card, per tick):
  - listing_count_now <= 0.70 * listing_count_24h_ago  (supply -30%)
  - price_now <= 1.03 * price_24h_ago                  (price not yet spiked)
...
All other recent strategies (floor_buy, post_dump, hourly_dip_revert) use
only the price series. listings_surge_v1 is the first to consume the
supply side — a "new lane" per the loop guardrails.
"""
import logging
...
class ListingsSurgeV1Strategy(Strategy):
    name = "listings_surge_v1"
    ...
    def __init__(self, params: dict):
        ...
        self.price_rise_max: float = params.get("price_rise_max", 1.03)
        # Exits
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            past_price = None
            for (hts, hp) in self._history[ea_id]:
                if hts <= ts_past:
                    past_price = hp
                else:
                    break
            # Walk back from end to find the most recent <= ts_past
            past_price = None
            ...
            if past_price is None or past_price <= 0:
                continue
            if price > self.price_rise_max * past_price:
                continue
