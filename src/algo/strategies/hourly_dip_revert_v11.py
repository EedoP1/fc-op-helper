"""Hourly dip reversion v11 — high-target + asymmetric risk + smaller positions.

v10 confirmed: under pessimistic execution, you NEED a big profit target to
clear the round-trip spread cost. Best v10 combo (+$414k) used profit_target=
0.15 with no volatility filter. But its W14 was -$106k because a few high-
priced losing trades (-$104k single trade) dragged it down.

v11 hypothesis:
  1. Bigger profit target (0.18) — only chase real, sustained moves.
  2. Tighter stop-loss (0.06) — cut losers FAST.
  3. Lower max_price (60k) — cap absolute coin exposure per card.
  4. More positions (8) — more diversification, smaller per-position risk.
  5. Stricter dip threshold (0.06) — fewer signals, higher conviction.

Risk-reward: +18% target / -6% stop. With even a 40% win rate the expected
value beats the pessimistic spread cost.
"""
import logging
...
class HourlyDipRevertV11Strategy(Strategy):
    """High-conviction big-target with asymmetric risk."""

    name = "hourly_dip_revert_v11"
    ...
    def __init__(self, params: dict):
        ...
        self.profit_target: float = params.get("profit_target", 0.18)
    ...
    def param_grid_hourly(self) -> list[dict]:
        ...
        combos = []
        for max_price in [50_000, 80_000, 150_000]:
            for profit_target in [0.15, 0.20]:
                for stop_loss in [0.05, 0.08, 0.12]:
