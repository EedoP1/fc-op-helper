"""Post-dump v18 — isolate qty_cap scale-up vs v15.

Minimal scale-up experiment: keep v15 EXACTLY as-is except bump qty_cap
from 6 -> 8. Tests whether sizing alone scales the edge, without the
confounds from v17 (which bumped max_positions, qty_cap, AND max_hold_h
simultaneously and crashed to -$49k, 1/6 bars).

Expected if edge is real: PnL scales ~1.33x (8/6), win rate roughly
holds (~80%), all 6 bars still pass. If bars break at this single-lever
bump, sizing itself is toxic at current liquidity — kill this line.

Every other param identical to post_dump_v15.
"""
import logging
...
class PostDumpV18Strategy(Strategy):
    name = "post_dump_v18"
