"""Floor buy v13 — v11 + post-stop cooldown (slope filter dropped).

v12 added both slope guard and stop cooldown; win rate crashed 60→39%
and correlation blew to -0.56. The slope guard was too aggressive,
rejecting good candidates along with bad ones.

v13 keeps v11 as base and adds ONLY the stop-cooldown: after a hard_stop
or smoothed stop_loss fires, the card cannot be re-bought for 48h.
This targets the specific failure mode seen in v11 (ea=252371 re-bought
after crash, crashed again).
"""
import logging
...
class FloorBuyV13Strategy(Strategy):
    name = "floor_buy_v13"
