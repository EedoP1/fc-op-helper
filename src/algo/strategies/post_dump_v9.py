...
class PostDumpV9Strategy(Strategy):
    name = "post_dump_v9"
    ...
    def __init__(self, params: dict):
        ...
        self.trickle_per_tick: int = params.get("trickle_per_tick", 1)
        self.trickle_fire_hour: int = params.get("trickle_fire_hour", 6)
        # Per-card filters
        ...
        self._trickle_remaining: int = 0
        self._last_trickle_date: str = ""
    ...
    def on_tick_batch(
        ...
        # Trigger: dumped X% over 48h AND just turned up over 6h
        d48 = self._g_delta(self.dump_lookback_h)
        d6 = self._g_delta(self.recovery_short_h)
        triggered = (d48 <= self.dump_min_pct and d6 >= self.recovery_min_pct)
        if triggered and self._trickle_remaining == 0:
            # Fresh trigger: load up trickle queue with full basket
            self._trickle_remaining = self.basket_size
        if self._trickle_remaining == 0:
            return signals
        # Only fire once per CALENDAR DAY (spread basket over consecutive days)
        today_key = ts_clean.strftime("%Y-%m-%d")
        if today_key == self._last_trickle_date:
            return signals
        # Also only fire on the chosen hour (early UTC morning, when EU market wakes)
        if ts_clean.hour != self.trickle_fire_hour:
            return signals
        ...
        # Trickle: only buy up to per_tick quota
        per_tick_quota = min(self.trickle_per_tick, self._trickle_remaining)
        basket = basket[:per_tick_quota]
        ...
        for ea_id, price, _ in basket:
            ...
            if qty > 0:
                ...
                available -= qty * price
                buys_made += 1
                self._trickle_remaining -= 1

        if buys_made > 0:
            self._last_trigger_ts = ts_clean
            self._last_trickle_date = today_key
