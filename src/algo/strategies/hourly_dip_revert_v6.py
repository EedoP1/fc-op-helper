"""Hourly dip reversion v6 — filter out thin-market volatile cards.

v5 ran clean (+3.75M) but 12% of trades booked >50% margins. Inspection shows
they're concentrated on low-price (12-20k) cards that swing wildly even on
median-of-hour prices. Those "dips" are real in the data but likely not
executable at qty=3 in live trading because the listings are thin.

v6 adds a card-level stability filter: reject any card whose 48h (max - min)
exceeds N% of 48h median. A stable meta card oscillates in a narrow band (the
band IS the edge); a wildly-swinging card is a lottery ticket, and over a
small sample the lotto will overstate backtest PnL.

Also bumps min_age_days (14 instead of 7) and min_price (15k instead of 10k)
to further cull new-card noise and cheap-card thinness.
"""
import logging
...
class HourlyDipRevertV6Strategy(Strategy):
    """v5 + 48h range/median volatility filter + stricter price/age floors."""

    name = "hourly_dip_revert_v6"
    ...
    def __init__(self, params: dict):
        ...
        self.min_price: int = params.get("min_price", 15000)
        ...
        # NEW: reject volatile cards. 48h (max-min)/median must be <= this.
        # 1.0 effectively disables the filter.
        self.max_range_pct: float = params.get("max_range_pct", 0.35)
    ...
    def _too_volatile(self, history: deque) -> bool:
        if len(history) < 12:
            return True
        vals = list(history)
        mn = min(vals)
        ...
        if med <= 0:
            return True
        return (mx - mn) / med > self.max_range_pct
    ...
    def param_grid_hourly(self) -> list[dict]:
        ...
        combos = []
        for max_range_pct in [0.30, 0.50, 1.0]:
            for min_age_days in [10, 14]:
