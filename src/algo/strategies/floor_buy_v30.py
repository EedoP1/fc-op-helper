"""Floor buy v30 — v21 band shifted UP (tests cohort-vs-narrow-band edge).

v21 baseline: +$502k org / 93.8% win / 16 trades / W14 $0 / W15 +$29k /
W16 +$473k (+47.3%). 4/6 bars. Fails bar 2a (W14=0) + bar 4 (corr -0.829).

Hypothesis: shift v21's narrow band UP to test whether v21's edge is
specifically about $10-13k cards (load-bearing cohort) or generalizes to
any narrow low-price band. Different cohort → different trade timing →
might populate W14 without widening the band.

Changes vs v21:
  min_price: 10000 → 14000
  floor_ceiling: 13000 → 17000
  floor_stable: 14500 → 18000
  week_max_ceiling: 18000 → 22000

All other knobs identical to v21 (profit_target 0.50, max_hold 240,
qty 10/18/25, burn_in 72, max_positions 12).
"""
import logging
...
class FloorBuyV30Strategy(Strategy):
    name = "floor_buy_v30"
    ...
    def __init__(self, params: dict):
        ...
        self.floor_stable: int = params.get("floor_stable", 18000)
