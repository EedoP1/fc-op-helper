"""Combo v10 — DEDICATED capital pools (v19: $700k, v23: $300k).

Midpoint ratio between combo_v8 (750/250) and combo_v9 (600/400).

Prior combo reference:
  - combo_v8 (v19 $750k + v23 $250k): 4/6 bars. corr -0.36 FAIL. uf/f 0.10 PASS.
  - combo_v9 (v19 $600k + v23 $400k): 4/6 bars. corr +0.028 PASS. uf/f 0.51 FAIL.

Hypothesis: midpoint 70/30 split targets corr roughly -0.15 to -0.20
(passes |corr| ≤ 0.30). v19 keeps $100k more than combo_v9 so uf/f
should drop back under 0.5 while corr stays bounded.

combo_v10 approach — DEDICATED arm-local cash counters:
  - v19 (floor_buy_v19 default params): budget $700k.
  - v23 (post_dump_v23 default params, DoW-gated trig_a): budget $300k.
...
class ComboV10Strategy(Strategy):
    name = "combo_v10"
    ...
    def on_tick_batch(
        ...
        # ── v19 arm (floor_buy_v19) — gated by its $700k local cash ──
        fb_signals_raw = self.floor_buy.on_tick_batch(
            ticks, timestamp, self._fb_arm,
        )

        # ── v23 arm (post_dump_v23) — gated by its $300k local cash ──
