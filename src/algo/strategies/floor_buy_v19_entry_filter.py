"""Floor buy v19 + bearish-signal ENTRY filter — iter 61.

Iter 59 and 60 tried using the iter-53 bearish signal (listings deplete
>=30% in 6h AND price range <5%) as an EXIT overlay on v19. Both hurt v19:
iter 59 cut 61% of trades and lost -$117k; iter 60 gated overlay behind
an underwater check and still lost -$192k vs v19.

Hard finding: per-card bearish signals cannibalize v19 because v19
specifically BUYS dumping-bottom cards that are STILL in a bearish
micro-regime. The exit overlay fires on exactly the cards v19 is waiting
to recover.

This iter FLIPS the signal: use it as a BUY REJECTION, not an exit.
Theory: v19's winners are cards where the dump has BOTTOMED. v19's
losers are cards where the dump is STILL ONGOING. The bearish signal
(77% directional down accuracy in iter 53 analysis) should separate
the two: if listings are depleting fast with a stable price right NOW,
the card is probably still dumping — skip it.

Hold discipline on existing positions is 100% preserved (nothing in the
exit logic changes). Only entries are filtered.

Chose the "now" variant (check current-hour bearish signal only) over
the "last 24h any-fired" variant for cleaner semantics and fewer false
positives.
"""
import logging
...
class FloorBuyV19EntryFilterStrategy(Strategy):
    name = "floor_buy_v19_entry_filter"
    ...
    def __init__(self, params: dict):
        ...
        # Bearish-signal knobs (same iter-53 definition as iter 59/60).
    ...
    def _bearish_signal_now(self, ea_id: int, ts_clean: datetime) -> bool:
        """Current-tick bearish signal: listings drop >=30% in 6h while price stable.

        Same iter-53 definition used in iter 59/60 absorb_exit overlays,
        but applied at the CURRENT hour as a buy-rejection filter.
        """
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            # NEW: bearish-signal entry filter — reject if card is currently
            # in a bearish micro-regime (listings depleting + price stable).
            if self._bearish_signal_now(ea_id, ts_clean):
                continue
