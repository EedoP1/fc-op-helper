"""Floor buy v20 — v19 + faster cycle (target 0.30, max_hold 168h).

v19 trade log: 15 trades, most winners hit +50% smoothed target at t+218h
(hours 218 out of 240h max_hold). Lower profit_target would exit earlier
and free capital for re-entry.

v20 swaps:
  - profit_target 0.50 → 0.30 (exit at +30% smoothed, earlier)
  - max_hold_h 240 → 168 (cap holds at 7 days, force cycle)

Smaller per-winner magnitude but potentially more cycles; test whether
reinvestment compounds before end of window.
"""
import logging
...
class FloorBuyV20Strategy(Strategy):
    name = "floor_buy_v20"
