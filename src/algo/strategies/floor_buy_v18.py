"""Floor buy v18 — v17 + 168h volatility guard.

v17 aggressive (+$305k / 76% win / 17 trades) but W15 still -3.1% driven
by ea=252371 (Bellingham-class). That card repeatedly:
  - passes 72h dwell (72h range fits the loose tier)
  - then crashes from local peak to hard_stop floor

Fix: require 168h (7-day) price range ≤ 25% for any non-small tier.
ea=252371's 7-day history swings ±30%+ (e.g. $11k → $16k → $11k → $14k),
which will fail this filter. Cards that are truly flat-floor for a full
week have range ≤ 20-25%.
"""
import logging
...
class FloorBuyV18Strategy(Strategy):
    name = "floor_buy_v18"
        ...
        for ea_id, price in ticks:
            ...
            if len(hist) >= self.week_window_h:
                ...
                if max(week) > self.week_max_ceiling:
                    continue
                # 168h volatility guard: skip cards with big weekly swings
