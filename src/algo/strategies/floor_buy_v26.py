"""Floor buy v26 — v19 + paired turnover regime shift.

v19 baseline: +$345.8k / 80% win / 3/6 bars, but W14 organic stuck at $0.
v22 (profit_target 0.30 alone) → 4/6, still W14 $0.
v25 (max_hold 96 alone) → 3/6, W14 -2.5%. Each single-axis move failed.

Hypothesis: PAIR the two turnover accelerators.
- profit_target: 0.50 → 0.25 (exit on modest bounce, don't wait for 50%)
- max_hold_h:  240 → 72  (force closure inside 3 days — populate W14/W15)

Theory: 3x–4x turnover with exits that actually trigger in-week might
populate W14 organically where neither change alone could. Tradeoff is
smaller per-trade PnL; expected ~0.7x–1.2x of v19's $345k if the
additional completed-trade count compensates for the shrunken margin.
All other params identical to v19.
"""
import logging
...
class FloorBuyV26Strategy(Strategy):
    name = "floor_buy_v26"
