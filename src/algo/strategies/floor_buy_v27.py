"""Floor buy v27 — v19 + 12h uptick momentum-confirmation filter.

v19 locked: +$345.8k / 80% win / W16 +35% / 3/6 bars.
Iter 13's -0.886 corr revealed "buy trailing-min + 2%" dip entries
systematically catch falling knives. Momentum confirmation (only buy
when price is already ticking UP) should reject the knives.

Hypothesis: require price_now >= 1.01 * price_12h_ago. If price has
not recovered at least 1% over the trailing 12 hours, the bounce has
not started — skip. Uses existing self._history[ea_id] deque; if
fewer than 12 entries available, skip until history is populated.

All other v19 entry/exit logic preserved identically.
"""
import logging
...
class FloorBuyV27Strategy(Strategy):
    name = "floor_buy_v27"
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            # v27 momentum-confirmation: price must be >=1% above 12h-ago value
            if len(hist) >= 12:
                price_12h_ago = list(hist)[-12]
                if price_12h_ago > 0 and price < price_12h_ago * 1.01:
                    continue
            else:
                continue  # Not enough history for momentum check
