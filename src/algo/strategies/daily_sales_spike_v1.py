"""Daily sales-velocity spike v1 (iter 63).

Hypothesis: when a card's daily total_sold_count on day D exceeds 2.0x its
rolling-5d mean, the next day (D+1) tends to see a positive price move. The
brief originally proposed 1.5x, but Phase 1 analysis (see
scripts/phase1_sales_spike_signal.py) showed that 1.5x is noise (median
next-day return +0.00%, frac>+2% = 47%) while 2.0x shows a clear edge
(median +5.75%, 61% of spikes return >+2%).

Signal trigger: during the day AFTER a spike day, if price is still in the
$10-30k floor band, buy. Exit on +6% target, -4% stop, or 48h max hold.

DB load at init uses the same threaded helper pattern as card_tier_v1 to
avoid nested asyncio.run from the backtester's event loop.
"""
    ...
    with _SALES_LOCK:
        ...
        async def _run():
            ...
            async with eng.connect() as c:
                # Pull margin=3 rows because total_* cols are same across margins
                r = await c.execute(
                    text(
                        """
        ...
        if "err" in holder:
            logger.error(f"daily_sales_spike_v1: DB load failed: {holder['err']}")
            _SALES_CACHE = {}
            return _SALES_CACHE
...
class DailySalesSpikeV1Strategy(Strategy):
    name = "daily_sales_spike_v1"
    ...
    def __init__(self, params: dict):
        ...
        # Floor-band gates (aligned with v19 / card_tier_v1 lineage)
        ...
        # Window for "post-spike day" — buy during the first H hours of day D+1
        ...
        logger.info(
            f"daily_sales_spike_v1: loaded sales for {len(self._ea_sales)} ea_ids, "
    ...
    def _is_post_spike_now(self, ea_id: int, ts: datetime) -> bool:
        ...
        if not spikes:
            return False
        # Current civil date from timestamp
        today = ts.date()
        # "Post-spike day" = day after a spike. Check D-1.
        # But we also allow a bit of slack so we can buy early-morning on day after spike.
        # Simplest rule: any spike-day within (now - post_spike_valid_h, now).
        cutoff = ts - timedelta(hours=self.post_spike_valid_h)
        for d in spikes:
            # The spike day D means trading opportunity exists on D+1.
            # We consider 'post-spike' if the spike was on any date in [cutoff.date(), today - 1 day]
            spike_end = datetime.combine(d + timedelta(days=1), datetime.min.time())
            if cutoff <= spike_end <= ts:
                return True
            # Also allow same-day (mid-spike ramp) if we want to front-run — keep conservative: off
        return False
    ...
    def on_tick_batch(
        ...
        candidates: list[tuple[int, int, int]] = []  # (ea_id, price, smooth)
        for ea_id, price in ticks:
            # Must have a recent spike
        ...
        # Sort cheapest first to maximize trade count for given capital
        candidates.sort(key=lambda x: x[1])

        # Estimate available cash with pending sell revenue
        sell_rev = 0
        for s in signals:
            if s.action == "SELL":
        ...
        for ea_id, price, _smooth in candidates:
            ...
            if available <= 0 or price <= 0:
                break
            # Target notional_per_trade, but cap by available cash
