"""Dual-trigger leader v1 — post_dump_v15 gating, leader-basket selection.

Keeps v15's dual-trigger regime detection (trig_a: global dump -3.5%/48h +
recovery +0.4%/6h; trig_b: promo-Friday-Sat alignment with -2.5%/24h) and
exit logic unchanged. Swaps basket selection from "6 cheapest liquid cards"
to "6 strongest-rallying cards" (per-card 24h delta >= +3%, sorted DESC).

Theory: v15's trigger fires at real market-regime shifts (orthogonal to
promo_dip_buy). Cheapness is arbitrary; riding leaders may compound better.
Risk: leader basket could correlate with promo_dip_buy momentum and break
the corr <= 0.30 bar.
"""
import logging
...
class DualTriggerLeaderV1Strategy(Strategy):
    name = "dual_trigger_leader_v1"
    ...
    def __init__(self, params: dict):
        ...
        # Leader basket — require 24h rally
        self.leader_min_24h_delta: float = params.get("leader_min_24h_delta", 0.03)
        # Exits
    ...
    def _card_24h_delta(self, ea_id: int) -> float | None:
    ...
    def on_tick_batch(
        ...
        # Trigger fires! Build basket of LEADER cards (strongest 24h rally)
        basket: list[tuple[int, int, int, float]] = []
        for ea_id, price in ticks:
            hist = self._history[ea_id]
            if len(hist) < 24:
                continue
            ...
            if created:
                ...
                if (ts_clean - cr_clean).days < self.min_age_days:
                    continue
            # Leader filter: require 24h rally
            d24 = self._card_24h_delta(ea_id)
            if d24 is None or d24 < self.leader_min_24h_delta:
                continue
            basket.append((ea_id, price, smooth, d24))

        if not basket or len(portfolio.positions) >= self.max_positions:
            return signals

        # Sort by 24h delta DESCENDING (strongest rally first)
        basket.sort(key=lambda x: x[3], reverse=True)
