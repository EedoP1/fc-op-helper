"""Floor buy v9 — v7 + 7-day price cap filter.

v7 (+$55k / 58.1% win) W15 negative, v8 (+$111k / 35.3% win) W14+W15 both
negative. Root cause suspect: cards that were premium a week ago AND
recently fell to floor look identical to long-time floor cards under a
72h-dwell filter, but they're in active downtrends (falling knives).

v9 adds a 7-day (168h) hard price cap: the card's maximum price over the
last week must be ≤ 7d_max_ceiling. True floor cards stay near floor for
weeks; ex-premium cards have prices well above this cap within 7 days.

Everything else is v7 (dwell-tiered sizing).
"""
import logging
...
class FloorBuyV9Strategy(Strategy):
    name = "floor_buy_v9"
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            # 7-day hard cap: if we have 168h of history, no price in that
            # window may exceed week_max_ceiling. Excludes ex-premium cards
            # that recently fell to floor (falling knives).
