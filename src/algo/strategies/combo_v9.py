"""Combo v9 — DEDICATED capital pools (v19: $600k, v23: $400k).

Tightens combo_v8's ratio by shifting $150k from v19 → v23. Hypothesis:
more v23 trades dilute v19's -0.83 corr signature further, pushing
|corr| below the 0.30 bar. Accept potentially marginal W14/W15 drag;
focus on unlocking bar 4 (|corr| ≤ 0.30) to reach 5/6.

Prior combo reference:
  - combo_v8 (v19 $750k + v23 $250k): +$311k, 75% win, 4/6 bars, corr -0.36.

combo_v9 approach — DEDICATED arm-local cash counters:
  - v19 (floor_buy_v19 default params): budget $600k.
  - v23 (post_dump_v23 default params, DoW-gated trig_a): budget $400k.
...
class ComboV9Strategy(Strategy):
    name = "combo_v9"
    ...
    def on_tick_batch(
        ...
        # ── v19 arm (floor_buy_v19) — gated by its $600k local cash ──
        fb_signals_raw = self.floor_buy.on_tick_batch(
            ticks, timestamp, self._fb_arm,
        )

        # ── v23 arm (post_dump_v23) — gated by its $400k local cash ──
