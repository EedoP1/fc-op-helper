"""daily_trend_dip_v5 — smoothed catastrophic-loss stop on v4 (iter 76).

v4 booked +$73.5k filtered / 67% win on 30 trades, but two craters drained
$121k combined:
    - ea_id=50595446: buy=49750 qty=2 hold=144h net=-$75,275
    - ea_id=50590888: buy=42750 qty=2 hold=144h net=-$46,075

v1's per-tick hard_stop=-15% killed positives by tripping on intra-hour loader
noise (BUY@max / SELL@min single-bar swings). v5 reintroduces a stop ONLY in
its smoothed form — and only at a depth that catches the craters but never
fires on v4's 28 non-crater trades.

Pre-analysis (this iter, hourly medians from market_snapshots, exit at MIN of
trigger bar with 5% EA tax):
    threshold N    predicted_pnl   clips
    -0.20     12   +96.7k          3 (one false-clip)
    -0.25     12   +123.8k         2 (catches both craters cleanly)
    -0.25     14   +128.1k         2 (best — same clips, slightly later exit)
    -0.27     14   +128.1k         2
    -0.30     14   +119.5k         2
    24h-avg -0.25 +95.8k           3

Chosen: smoothed_stop=-0.25 with N=14 consecutive hourly closes BELOW
buy_price * 0.75 (median of last 3 ticks per stored daily smoother). The 14h
window guarantees the stop reflects sustained breach, not a single bad bar.
Predicted v5 PnL: +$128k (vs v4 +$73.5k → Δ +$54.6k).

Validation: simulating the stop on every v4 trade, exactly 2 trades trigger
(the two known craters at hr 55-56 of their 144h hold). Zero v4 winners are
clipped. So the realised v5 PnL should land ~$120-128k filtered (small drift
possible if the stopped slots free capital that v4 couldn't deploy).

All other params identical to v4: dual band ($<=20k OR $40-60k), trend_3d <=
-0.05, op_demand@10 >= 1.5, whitelist 86-91 + repeater types, profit_target
+20%, max_hold_h=144, sizing 12 slots / qty_cap 8 / notional $100k.
"""
...
def _load_daily_sync() -> tuple[dict, dict]:
    ...
    with _DAILY_LOCK:
        ...
        if "err" in holder:
            logger.error(f"daily_trend_dip_v5: DB load failed: {holder['err']}")
            _DAILY_CACHE = ({}, {})
        else:
...
class DailyTrendDipV5Strategy(Strategy):
    name = "daily_trend_dip_v5"
    ...
    def __init__(self, params: dict):
        ...
        # Exits — profit_target + max_hold + smoothed_stop (NEW in v5).
        self.profit_target: float = params.get("profit_target", 0.20)
        self.max_hold_h: int = params.get("max_hold_h", 144)
        # SMOOTHED stop: N consecutive hourly smoothed prices BELOW buy*(1-stop).
        # Per pre-analysis (iter 76): -0.25 / N=14 catches both v4 craters cleanly,
        # zero false-positives on v4 winners. Predicted PnL +$128k (vs v4 +$73.5k).
        ...
        logger.info(
            f"daily_trend_dip_v5: daily totals for {len(self._daily_t)}, "
        ...
        # NEW: per-position consecutive-hour breach counter for the smoothed stop.
        self._stop_breach_count: dict[int, int] = {}
        # Track the last hour we updated the breach counter so we only count once
        # per stored hour (avoids double-counting if engine ticks in sub-hour batches).
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            if holding <= 0:
                # housekeeping: drop stale state for closed positions
            ...
            if hold_hours >= self.max_hold_h:
                ...
                if sm > 0 and buy_price > 0:
                    ...
                    if pct >= self.profit_target:
                        ...
                        # Smoothed catastrophic stop: count consecutive distinct
                        # hourly buckets where smoothed price <= buy * (1 - stop).
                        # Only update the counter once per new hour to avoid double-
