"""Floor buy v5 — v4 + scaled position sizing.

v4 (qty 8, positions 12): +$49.8k, 56.1% win (PASSES ≥55% bar), 41 trades,
corr −0.307 (just over |r|≤0.30 bar).

Scaling test: qty_cap 8 → 10 only. One change from v4. If PnL scales ~25%,
we reach ~$62k; if it scales closer to 1:1 with qty, we may hit $75-80k.
"""
import logging
...
class FloorBuyV5Strategy(Strategy):
    name = "floor_buy_v5"
