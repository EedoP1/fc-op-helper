...
class PostDumpV10Strategy(Strategy):
    name = "post_dump_v10"
    ...
    def __init__(self, params: dict):
        ...
        # Trigger B: deep ongoing dump
        self.deep_dump_h: int = params.get("deep_dump_h", 36)
        self.deep_dump_pct: float = params.get("deep_dump_pct", -0.05)
    ...
    def on_tick_batch(
        ...
        # Trigger B: deep ongoing dump (no recovery required) — fires on
        # SAT after Friday promo flood, breaking anti-correlation with
        # promo_dip_buy by deliberately overlapping dump-day fires.
        d36 = self._g_delta(self.deep_dump_h)
        trig_b = (d36 <= self.deep_dump_pct)
