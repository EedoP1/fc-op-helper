"""Floor buy mega v1 — iter 51 mega-position recycling.

Opposite direction from iter 50 (turnover_v1): instead of MORE slots with
smaller qty, use FEWER slots with MUCH larger qty. From brief:
"qty=50 per card, $600k positions... As fast-cycle arm sells at +8%,
immediately redeploy + compound."

Math: 5 positions x qty=50 x $12k = $3M notional (3x leverage via
sell-recycling). At 8%/96h cycle, 5 positions x 3 cycles/week x $48k =
~$720k/week -> $2M in 3 weeks.

Risk: market depth for 50-unit positions at --min-sph 2 (48 sales/day) is
borderline. If fills are consistently < requested qty_large=50, mega is a
hard ceiling and we learn liquidity is the bottleneck.

Base: v36 (8%/96h fast-cycle). Only position + qty params changed,
quality filters untouched.
"""
import logging
...
class FloorBuyMegaV1Strategy(Strategy):
    name = "floor_buy_mega_v1"
