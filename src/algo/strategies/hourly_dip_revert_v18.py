"""Hourly dip reversion v18 — multi-target partial-sell exit.

v12 exits entire position on first of: +25% smoothed target, -15% smoothed
stop, 48h max-hold. Win rate 61.3% — many winners rally to +15-20% then
reverse back through break-even into stop territory. Those trades earn 0
or negative instead of the +15% they "visited."

v18 adds an intermediate partial-sell target: when smoothed reaches
`partial_target` (e.g. +12%), sell HALF the position. Remaining half rides
to +25% target or gets stopped out. Positions start with qty_cap=4 to
allow clean halving.

Expected effect:
- Trades that peak at 12-20% now book partial profit instead of
  round-tripping through stop.
- Trades that reach +25% target book slightly less on the full move
  (half at +12% instead of +25%) but the +12% half is still profitable.
- Trades that stop out immediately book half the initial position at -15%
  instead of full — but stops fire before the partial target in this path
  so no difference.

Net hypothesis: boost effective win rate from 61% toward 70-75% (because
"partial booked" counts as a win) without materially impacting losing
trades.
"""
import logging
...
class HourlyDipRevertV18Strategy(Strategy):
    """v12 entry, multi-target partial-sell exit."""

    name = "hourly_dip_revert_v18"
    ...
    def __init__(self, params: dict):
        ...
        self.partial_target: float = params.get("partial_target", 0.12)
        self.partial_frac: float = params.get("partial_frac", 0.5)
        ...
        self.qty_cap: int = params.get("qty_cap", 4)  # 4 so halving gives clean 2
        ...
        self._partial_sold: set[int] = set()
            ...
            if holding > 0:
                ...
                if hold_hours >= self.max_hold_h:
                    ...
                    if smooth_pct >= self.profit_target:
                        ...
                        full_sell = True
                    elif (smooth_pct >= self.partial_target
                          and ea_id not in self._partial_sold):
                        partial_sell = True
                ...
                if full_sell:
                    ...
                    self._partial_sold.discard(ea_id)
                elif partial_sell:
                    half = max(1, int(holding * self.partial_frac))
                    if half < holding:
                        signals.append(Signal(action="SELL", ea_id=ea_id, quantity=half))
                        self._partial_sold.add(ea_id)
