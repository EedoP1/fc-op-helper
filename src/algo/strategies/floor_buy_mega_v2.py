"""Floor buy mega v2 — iter 52 true-test of direction (B) aggressive recycling.

Combines v19's proven 50%/240h harvest discipline (which captured +$473k
in W16 alone) with mega position sizing AND relaxed vol_range gates so
the qty_large=50 tier actually deploys on realistic candidates.

Iter 51 (floor_buy_mega_v1) revealed that v19's default
vol_range_tight=0.10 / vol_range_loose=0.20 rarely activate in the
real tape, so qty=40/50 sizes fired only a handful of times, effectively
degrading mega_v1 back to a 5-slot qty=25 portfolio. On top of that,
mega_v1 used v36's 8%/96h fast-cycle exits which gave up the W16 tail.
mega_v2 fixes BOTH issues: widen vol gates to 0.18/0.30 so mega tier
genuinely fires, and keep v19's 50%/240h harvest discipline so we ride
the W16 pump instead of scalping it.

Math: 5 slots x qty_large=50 x $12k = $3M notional. If 3/5 positions
hit v19's 50% target in W16 alone: 3 x 50 x $12k x 0.50 x 0.95 = +$855k
(minus 2 stopped-out losers at ~$60k each = +$735k net from W16).

Risk: concentration. At 5 slots only, a sudden market reversal can wipe
the portfolio. stop_loss=0.10 and hard_stop=0.15 protect against this,
but at qty=50 each stop costs 50 x $12k x 0.10 = $60k per position.

Base: floor_buy_v19. Only max_positions, qty_*, and vol_range_* changed.
"""
import logging
...
class FloorBuyMegaV2Strategy(Strategy):
    name = "floor_buy_mega_v2"
    ...
    def __init__(self, params: dict):
        ...
        self.vol_range_tight: float = params.get("vol_range_tight", 0.18)
        self.vol_range_loose: float = params.get("vol_range_loose", 0.30)
        ...
        self.qty_small: int = params.get("qty_small", 25)
        self.qty_medium: int = params.get("qty_medium", 40)
        self.qty_large: int = params.get("qty_large", 50)
    ...
    def param_grid_hourly(self) -> list[dict]:
        return [{
            ...
            "hard_stop": 0.15,
            "stop_cooldown_h": 48,
            "vol_range_tight": 0.18,
            "vol_range_loose": 0.30,
            "max_hold_h": 240,
            "min_price": 10000,
            "max_positions": 5,
            "min_age_days": 7,
