"""Floor buy v7 — v4 base + dwell-tiered sizing.

v4 (24h stability, qty 8): +$49.8k, 56.1% win (pass), corr −0.307, low PnL.
v6 (72h stability, qty 8): +$140k (pass), 39.0% win (fail), bigger per-trade wins.

The 72h-dwell candidates generate much bigger wins AND bigger losses.
24h-dwell candidates produce consistent small wins, higher hit rate.

v7 accepts 24h-stable entries (matching v4), but sizes them differently
based on how long the card has been sitting ≤ floor_stable:
  - dwell ≥ 72h: qty_large (high conviction, 72h-floor-dwell cards)
  - dwell ≥ 24h but < 72h: qty_small (consistent, 24h-dwell fallback)

This mirrors v20's dip-depth tiering for the floor domain: commit capital
where the signal is strongest, take low-conviction trades at reduced size.
"""
import logging
...
class FloorBuyV7Strategy(Strategy):
    name = "floor_buy_v7"
    ...
    def __init__(self, params: dict):
        ...
        # Keep full history for dwell assessment up to the large window
    ...
    def on_tick_batch(
        ...
        candidates: list[tuple[int, int, int, int]] = []  # (ea, price, smooth, qty)
        for ea_id, price in ticks:
            ...
            # Minimum stability: last 24h all ≤ floor_stable
            ...
            # Large-dwell tier: last 72h all ≤ floor_stable (requires 72h of history)
        ...
        # Sort: prefer large-tier candidates first, then by smoothed price asc
