"""Post-dump v25 — v15 verbatim with burn_in_h 72→24 (no DoW gate).

Combo v12 leaves W14 thin because v24's DoW gate (Sun/Mon/Tue) blocks
mid-week W14 fires. Hypothesis: drop the DoW gate entirely on a new
short-burn arm — v25 is post_dump_v15 verbatim (no DoW gate) with
burn_in_h reduced from 72 to 24 so it activates early enough to catch
the W14 window.

Entry: global dump-then-recovery (trig A) OR promo-Sat alignment (trig B).
Any weekday.

Short-burn + any-weekday pairs well in combo_v13 as the $100k arm alongside
v19 ($650k floor harvest) and v23 ($250k corr support).
"""
import logging
...
class PostDumpV25Strategy(Strategy):
    name = "post_dump_v25"
