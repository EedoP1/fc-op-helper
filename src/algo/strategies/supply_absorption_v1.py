"""Supply absorption v1 — listing depletion WITH price stability.

Hypothesis: when `listing_count` drops sharply (>=30% in 6h) while price stays
flat (<5% range over the same window), buyers are absorbing supply faster than
sellers restock — a pre-spike signal orthogonal to floor/dump/promo triggers.

Entry (BUY):
- lc_6h >= 20 AND (lc_6h - lc_now)/lc_6h >= 0.30
- 6h price range / median < 0.05
- 10k <= price <= 50k
- min_age_days = 7
- burn_in 24h
...
Exit (SELL):
- +8% profit target (vs buy_price smoothed via last tick)
- -5% stop loss
- 72h max hold (force stale exit)
"""
import logging
...
class SupplyAbsorptionV1Strategy(Strategy):
    name = "supply_absorption_v1"

    def __init__(self, params: dict):
        self.params = params
        self.depletion_threshold: float = params.get("depletion_threshold", 0.30)
        self.min_lc_6h: int = params.get("min_lc_6h", 20)
        self.price_stability_range: float = params.get("price_stability_range", 0.05)
        ...
        # Rolling price history per card for 6h price stability check
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            if price_range / med >= self.price_stability_range:
                continue
