"""Post-dump v16 — scale v15's output by enlarging basket and position caps.

v15 passes all 6 verdict bars (+$144.6k, 83% win, corr +0.067) but weekly
returns (~3%/wk) fall well short of the 25%/wk target. v15's signal
triggers 4-ish times per window but only invests a small fraction of the
$1M budget each fire (6 cards × 6 qty × ~$15k ≈ $540k — but allocated at
most 12 positions at once).

Hypothesis: the signal is already strong; scaling basket breadth and the
position cap should scale PnL roughly linearly toward the 25%/wk goal
WITHOUT moving the signal or changing win rate.

Knobs scaled (size only — NO signal threshold changes):
  - basket_size: 6 → 18 (3x)  — more cards per fire, wider diversification
  - max_positions: 12 → 36 (3x) — raise concurrency cap so the larger
    basket actually gets held
  - qty_cap: left at 6 (per-card quantity unchanged — scaling this risks
    illiquidity and reduces effective win rate)

Capital check: 18 cards × 6 qty × ~$15k avg = ~$1.62M notional per fire,
which exceeds $1M — the buy loop's `available` accounting will naturally
truncate once cash is exhausted, so we allocate as much as possible per
fire without breaking the budget guard.

Knobs considered but RULED OUT:
  - qty_cap 6 → 18 (3x per-card size): rejected — per-card allocation is
    already limited by liquidity; larger per-card qty can't actually
    execute and skews towards illiquid losers.
  - Relaxing signal thresholds (dump_min_pct, recovery_min_pct,
    cooldowns): rejected — the whole point is to keep v15's signal
    intact; moving thresholds would invalidate the bar-pass result.
  - Extending max_hold_h: rejected — longer holds increase correlation
    with global market drift and can regress the corr-vs-promo_dip_buy
    bar.
"""
import logging
...
class PostDumpV16Strategy(Strategy):
    name = "post_dump_v16"
    ...
    def param_grid_hourly(self) -> list[dict]:
        # v15 param_grid copy, with basket_size 6→18 and max_positions 12→36
        # All signal-threshold knobs unchanged from v15.
        return [{
            "dump_lookback_h": 48,
            "dump_min_pct": -0.035,
            "recovery_short_h": 6,
            ...
            "basket_size": 18,          # v15: 6  → 3x breadth per fire
            "smooth_window_h": 3,
            "outlier_tol": 0.06,
            "profit_target": 0.15,
            "stop_loss": 0.10,
            ...
            "min_age_days": 7,
            "burn_in_h": 72,
            "qty_cap": 6,               # per-card qty unchanged
            "max_positions": 36,        # v15: 12 → 3x concurrent cap
        }]
