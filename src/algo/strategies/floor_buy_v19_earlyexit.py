"""Floor buy v19 + peak-tracking early-exit.

v19 only sells at +50% target, -10% stop-loss, -15% hard-stop, or 240h hold.
Many positions hit +15-30% gains that then unwind back to 0 or negative by
the 240h deadline, giving back real profit. This variant tracks the max
(3h-smoothed) price since buy, and if the smoothed price drops >=2% from
that peak AND the position was ever green, SELL.

Theory: v19 at +$502k is an undershoot — peak-locking could push to $700-900k.
"""
import logging
...
class FloorBuyV19EarlyExitStrategy(Strategy):
    name = "floor_buy_v19_earlyexit"
    ...
    def __init__(self, params: dict):
        ...
        # Early-exit: if smoothed price <= peak * (1 - peak_drawdown)
        # AND peak_price > buy_price, sell.
        self.peak_drawdown: float = params.get("peak_drawdown", 0.02)
        ...
        # Peak smoothed price since buy, per ea_id.
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            if holding > 0:
                ...
                # Track peak smoothed price since buy.
                if smooth > 0:
                    prev_peak = self._peak_prices.get(ea_id, buy_price)
                    self._peak_prices[ea_id] = max(prev_peak, smooth)

                sell = False
                stopped = False
                # Hard stop first — raw price crash below buy.
                if buy_price > 0 and price <= buy_price * (1.0 - self.hard_stop):
                    sell = True
                    stopped = True
                # Early-exit: peak tracked drawdown on smoothed price,
                # only if we were ever green. Runs BEFORE max-hold & target.
                elif (
                    smooth > 0
                    and buy_price > 0
                    and self._peak_prices.get(ea_id, 0) > buy_price
                    and smooth <= self._peak_prices[ea_id] * (1.0 - self.peak_drawdown)
                ):
                    sell = True
        ...
        for ea_id, price, _, qty_cap in candidates:
            ...
            if qty > 0:
                ...
                # Initialize peak at buy price — we haven't observed a higher
                # smoothed price yet; this prevents immediate early-exit
                # firing before the position has a chance to move green.
