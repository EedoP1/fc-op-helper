"""Floor buy v1 — buy cards trading near the $10-12k BIN floor.

Analysis of per-card 22-day drift on the --min-sph 5 universe:
  Top 10 drifters (UP): all started 10-13k, ran to 18-42k (+62% to +213%).
  Bottom 10 drifters (DOWN): all started 54k-500k, crashed to 13-77k (-77%
    to -85%). Premium cards bleed value as new content arrives weekly.

Root cause (FUT market dynamic):
  - $10-12k is the practical BIN floor on FC26. Below that, content packs
    flood supply at nil price — demand never catches up. Near floor = very
    limited downside + upside unlocked when supply tightens naturally.
  - Premium cards decay as weekly promos replace the meta. Dip-reversion
    fails on them because "dip" is really permanent devaluation.

Signal: BIN <= floor_ceiling AND has been <= floor_stable for recent_h hours
(confirms the card is hovering near floor, not free-falling). NO prior
rapid rise (would mean supply already tightening).

Exit: profit_target +30% on smoothed vs buy, stop_loss -10% on smoothed,
max_hold 96h (4 days — longer than dip-revert because floor-bounce takes
longer to play out).
"""
import logging
...
class FloorBuyV1Strategy(Strategy):
    """Buy cards near the $10-12k floor; hold for slow upward drift."""
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            # Entry: smoothed price near floor, recent hours also near floor
            ...
            if min(recent) < self.min_price * 0.9:
                # Guard against scanner crash-to-zero artifacts
                continue
            ...
            # Prefer the cheapest candidates first
        ...
        candidates.sort(key=lambda x: x[2])  # lowest smoothed first
