"""Floor buy v17 — aggressive: bigger qty, longer hold, higher target.

v15 hit +$120.6k / 12% over 22d. Target now: +25%/week ≈ +$500k over 22d.
Oracle analysis showed top drifters go +60-200% over the window; v15's
96h max-hold exits winners at +20-25% gross, ~10% of the available drift.

v17 rebalances for harvest-mode:
  - max_hold_h 96 → 240 (10 days; capture more of the drift curve)
  - profit_target 0.30 → 0.50 (let winners run to +50% smoothed)
  - qty_large 12 → 20 (size up conviction trades)
  - qty_medium 8 → 14
  - qty_small 4 → 8
  - max_positions 12 → 8 (fewer concurrent, each bigger)

Risk: longer hold + bigger qty = deeper drawdowns on crashes (v15's
Bellingham-class ea=252371 would lose ~2.5x more). The hard_stop at -15%
still caps tail risk per trade.
"""
import logging
...
class FloorBuyV17Strategy(Strategy):
    name = "floor_buy_v17"
    ...
    def __init__(self, params: dict):
        ...
        self.qty_small: int = params.get("qty_small", 8)
        self.qty_medium: int = params.get("qty_medium", 14)
        self.qty_large: int = params.get("qty_large", 20)
