"""Floor buy v15 — v13 base + 3-tier sizing by 72h range.

v13 hit PnL+Win+Corr bars but failed W15 (bar #2, both-weeks-profitable).
v14 flipped the trade-off: fixed W15 but broke W14 and win-rate.

v14's full downgrade of volatile cards from qty=12 → qty=4 is too blunt.
v15 introduces a MEDIUM tier between the two:
  - 72h range ≤ 10% : qty_large   = 12  (genuinely flat floor)
  - 72h range ≤ 20% : qty_medium  = 8   (bouncy floor, hedge size)
  - otherwise       : qty_small   = 4   (24h stable only)

Cards like ea=252371 (bouncy-floor premium) get qty_medium. Losses capped
at 2/3 of qty_large; winners captured at 2/3 rate. Net smoothing.

Keeps v13's 48h stop-cooldown (NOT v14's 72h any-sale cooldown). The
48h cooldown caught the ea=252371 re-buy after crash; the 72h after any
sale was too aggressive and drove correlation above bar.
"""
import logging
...
class FloorBuyV15Strategy(Strategy):
    name = "floor_buy_v15"
