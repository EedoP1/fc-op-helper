"""daily_trend_dip_v3 — surgical price band on v2 (iter 74).

v2 (-$91k, 60.5% win, +$35k unfiltered) showed real signal but bled $240k
across 7 max-hold drains, mostly clustered in mid-to-high price tiers.

Pre-analysis on v2's 38 filtered trades (iter 74):
    Bucket           n   pnl       win   avg_bp
    $<=20k           8   +$182k    75%   $15.7k
    $20-40k          9   -$137k    66.7% $28.0k   <-- hidden losers
    $40-60k          6   +$132k    83.3% $49.5k
    $60-80k          9   -$138k    33.3% $66.3k   <-- main drain
    $80k+            6   -$70k     50%   $101.1k  <-- secondary drain

Best price band: $25,000 <= buy_price <= $50,000
    n=10  pnl=+$249k  win=90%
Predicted v3 PnL upper bound: +$249k.

v19 overlap on band: 0/9 ea_ids — fully orthogonal.

v3 == v2 + post-filter [25_000 <= price <= 50_000]. Same gate, same exits,
same whitelist. Sizing tuned to spec: 8 slots × $125k notional, qty cap 50.
"""
...
def _load_daily_sync() -> tuple[dict, dict]:
    ...
    with _DAILY_LOCK:
        ...
        if "err" in holder:
            logger.error(f"daily_trend_dip_v3: DB load failed: {holder['err']}")
            _DAILY_CACHE = ({}, {})
        else:
...
class DailyTrendDipV3Strategy(Strategy):
    name = "daily_trend_dip_v3"
    ...
    def __init__(self, params: dict):
        ...
        # Iter 74: tighten band to v2's profitable cluster.
        self.min_price: int = params.get("min_price", 25000)
        ...
        # Sizing — spec: 8 slots × $125k qty cap 50.
        ...
        logger.info(
            f"daily_trend_dip_v3: daily totals for {len(self._daily_t)}, "
