"""Floor buy v21 — v19 params + max_positions 8 → 12 (entry-side widen).

v19 diagnostic (sell_time, includes boundary):
  W14 org: $0 (no W14 sells)   W15 org: -$3.9k   W16: +$349.7k (all boundary)

v19 issued only 15 trades across the window (3 W13 + 5 W14 + 7 W15 buys).
Per-week sells land in W15/W16, so W14 %/wk target is impossible with
that signal rate. Iter 1 (v20) clipped the exit; that hurt. This iter
instead widens entry capacity without loosening quality filters:

  max_positions 8 → 12

Rationale: v19 qty_large=25 with 8 slots caps concurrent notional.
Clustered W14 signals hit the slot lid and are dropped. v15 ran 12
slots with smaller qty (12) and was a champion. Keeping v19 qty but
raising the slot count admits more concurrent holdings when the floor
cohort has several bottoming cards at once — same quality gate, more
throughput. All other entry/exit knobs unchanged from v19.
"""
import logging
...
class FloorBuyV21Strategy(Strategy):
    name = "floor_buy_v21"
