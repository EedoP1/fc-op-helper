"""Timezone arbitrage v2 — circadian + day-of-week gate.

V1 was catastrophic (-$859.1k, 22.8% win over 177 trades): night-entry hours
(01-05 UTC) coincided with the Thu/Fri global dump regime, so every leg caught
the ride down.

Hypothesis for v2: circadian bias may still exist, but it is dominated by the
WEEKLY regime. Per post_dump_v15's EDA, Sun/Mon/Tue are the post-dump recovery
phase. Restricting entries to weekday in {Sun=6, Mon=0, Tue=1} should filter
out the dump-regime carnage and let the (supposed) night-dip signal surface on
the up-ramp days.

All other entry conditions and exit logic remain identical to v1.
"""
import logging
...
class TimezoneArbV2Strategy(Strategy):
    name = "timezone_arb_v2"
    ...
    def on_tick_batch(
        ...
        # ── Entries (only in entry window AND on Sun/Mon/Tue) ──
        if hour not in self.entry_hours:
            return signals
        if ts_clean.weekday() not in (6, 0, 1):
            return signals
