"""Combo v15 — TRIPLE-arm complementary-weeks stack (v19 $600k + v36 $250k + v23 $150k).

Iter 40 breakthrough: v36 standalone (max_hold 96h, profit_target 8%) posts
W14 +$65k (bar 2a PASS) and W15 +$14k (close), while v19 standalone owns W16
(+$473k) but contributes $0 to W14. Complementary week profiles → stack them.

Three-arm:
  - v19 (FloorBuyV19Strategy):   $600k — primary W16 harvest
  - v36 (FloorBuyV36Strategy):   $250k — W14/W15 fast-cycle contributor
  - v23 (PostDumpV23Strategy):   $150k — corr support (DoW-gated trig_a)

Cannibalization risk: v36 and v19 share the $10-13k floor band, but v36 exits
at 8% target / 96h max_hold while v19 holds to 50%/240h — different trade
styles on the same cards, v36 recycles capital through W14/W15 while v19
accumulates into W16. BUY dedupe still applies; v19 gets priority.

Each arm has its OWN dedicated cash pool via `_DedicatedCashArm`. Sells
recycle into the same arm only. BUY priority: v19 → v36 → v23. Duplicate
BUYs dropped and cash refunded.
"""
import logging
...
from src.algo.strategies.floor_buy_v36 import FloorBuyV36Strategy
...
class ComboV15Strategy(Strategy):
    name = "combo_v15"
    ...
    def __init__(self, params: dict):
        ...
        fb36_params = dict(FloorBuyV36Strategy({}).param_grid_hourly()[0])
        pd23_params = dict(PostDumpV23Strategy({}).param_grid_hourly()[0])

        self.floor_buy_v19 = FloorBuyV19Strategy(fb19_params)
        self.floor_buy_v36 = FloorBuyV36Strategy(fb36_params)
        ...
        self._fb36_arm = _DedicatedCashArm(None, self._fb36_ids, V36_BUDGET)
    ...
    def set_listing_counts(self, listing_counts: dict):
        ...
        self.floor_buy_v36.set_listing_counts(listing_counts)
    ...
    def on_tick_batch(
        ...
        self._fb36_arm._shared = portfolio
        ...
        # ── v36 arm — gated by its $250k local cash ──
        fb36_signals_raw = self.floor_buy_v36.on_tick_batch(
            ticks, timestamp, self._fb36_arm,
        )
        ...
        for s in fb36_signals_raw:
            p = price_by_id.get(s.ea_id, 0)
            if s.action == "BUY" and p > 0:
                self._fb36_arm._arm_ids.add(s.ea_id)
                self._fb36_arm._local_cash -= p * s.quantity
            elif s.action == "SELL" and p > 0:
                self._fb36_arm._local_cash += (p * s.quantity * 95) // 100
        ...
        # ── Merge: v19 → v36 → v23 priority; dedupe same-tick BUYs ──
        out: list[Signal] = []
        buy_ids: set[int] = set()
        ...
        for s in fb36_signals_raw:
            if s.action == "BUY":
                if s.ea_id in buy_ids:
                    # Duplicate with v19's BUY — drop and refund v36.
                    p = price_by_id.get(s.ea_id, 0)
                    if p > 0:
                        self._fb36_arm._local_cash += p * s.quantity
                    continue
        ...
        for s in pd23_signals_raw:
            if s.action == "BUY":
                if s.ea_id in buy_ids:
                    # Duplicate with v19/v36's BUY — drop and refund v23.
                    p = price_by_id.get(s.ea_id, 0)
                    if p > 0:
                        self._pd23_arm._local_cash += p * s.quantity
                    continue
