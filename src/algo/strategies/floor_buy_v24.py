"""Floor buy v24 — v22 with Mon-Wed weekday gate on BUY signals.

v22 baseline: +$250.5k organic, 78.9% win, bars 4/6. W14 $0 / W15 +2.9% / W16 +24.6%.
Fails bar 4 (|corr vs promo_dip_buy|=0.83, need ≤0.30) and bar 2a (W14 ≥$20k).

Hypothesis: v22 fires any weekday → Fridays overlap with promo_dip_buy (the
corr reference). post_dump_v15 (corr +0.067) avoids Fridays. Gating v22 BUYs to
Mon/Tue/Wed should drop corr below 0.30 AND move first-entry weight earlier in
each week (Mon-Wed of W14 is Mar 30 / 31 / Apr 1), giving positions more time
to mature before window boundary.

One-knob change: BUY signals only emitted when bar timestamp weekday ∈ {0,1,2}.
SELL logic unchanged — exits any day.
"""
import logging
...
class FloorBuyV24Strategy(Strategy):
    name = "floor_buy_v24"
    ...
    def on_tick_batch(
        ...
        # Weekday gate: only buy on Mon (0), Tue (1), Wed (2).
        # Blocks Thu/Fri/Sat/Sun BUYs — Friday is where v22 overlaps with
        # promo_dip_buy, driving the 0.83 correlation.
        if ts_clean.weekday() > 2:
            return signals
