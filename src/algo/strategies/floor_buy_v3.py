"""Floor buy v3 — v1 base + wider floor ceiling.

v1 (13k ceiling / 96h hold / qty 8): +$56k, 53.5% win, 43 trades.
v2 (14k ceiling / 168h hold / qty 12 / 15% stop): +$9k, 30.4% win, 23 trades.
  — Too many params changed at once; longer hold blocked reentry, bigger
    qty and wider stop amplified losses.

v3 keeps v1 unchanged except for floor_ceiling 13000 → 14000 (and matching
floor_stable). Tests if the wider candidate pool alone helps, without the
hold/size/stop changes that hurt v2.
"""
import logging
...
class FloorBuyV3Strategy(Strategy):
    name = "floor_buy_v3"
