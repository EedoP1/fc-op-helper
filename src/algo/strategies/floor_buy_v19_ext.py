"""Floor buy v19_ext — v19 microstructure filters shifted to $13-18k band.

Hypothesis (iter 5, drawdown-reversion loop): v19's edge lives in its
microstructure filters (72h range bound, week_max_ceiling, listing-structure,
stable floor), not purely in the $10-13k price band. If true, applying v19's
microstructure rules to a shifted higher price band should find orthogonal
trades. Stacking v19 + v19_ext should beat v19 alone (+$480k org) toward the
$2M target. Success criterion: stacked >= +$600k organic.

Only price-tier numbers are scaled; everything else (fractions, quantities,
hold windows, cooldowns, burn-in, promo filter, age filter) is identical to
floor_buy_v19.py so the two strategies operate in orthogonal price bands with
the same logic.

Scaling (13k -> 18k band):
  floor_ceiling     13000 -> 18000   (primary price cap)
  floor_stable      14500 -> 20000   (smoothed-ceiling proportional)
  week_max_ceiling  18000 -> 25000   (168h high cap)
  min_price         10000 -> 13000   (band floor)
"""
import logging
...
class FloorBuyV19ExtStrategy(Strategy):
    name = "floor_buy_v19_ext"

    def __init__(self, params: dict):
        self.params = params
        self.smooth_window_h: int = params.get("smooth_window_h", 3)
        self.outlier_tol: float = params.get("outlier_tol", 0.08)
        self.floor_ceiling: int = params.get("floor_ceiling", 18000)
        self.floor_stable: int = params.get("floor_stable", 20000)
        ...
        self.week_max_ceiling: int = params.get("week_max_ceiling", 25000)
