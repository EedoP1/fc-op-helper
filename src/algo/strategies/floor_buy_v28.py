"""Floor buy v28 — v21 + fast re-entry cooldown for profitable cards.

v21 passes 4/6 bars but fails W14 ($0 organic) and corr (-0.829). The
W14 miss is a trade-frequency issue: v21 issues only 15 buys across
the full window. The stop_cooldown_h=48 blocks re-entry for BOTH stop-outs
and profitable exits, so cards that just printed a clean buy→bounce→sell
cycle have to wait 48h before the algo can buy them again — even though
they're validated winners already.

v28 adds a PARALLEL `_profit_cooldown_until` dict tracked independently
from `_stopped_until`. Profit-target exits use a 24h cooldown (instead
of 48h) so validated winners can re-enter faster when price dips back
toward the floor. Stop-outs retain the 48h cooldown.

Logic: cooldown_end = max(stopped_until, profit_until) — whichever is
longer wins. If a card both stopped and then profited, the later cooldown
applies. Dict lookup uses `ts` as a default so "never set" maps to "no
cooldown active."

Hypothesis: populates W14 with additional trades from cards that v21
already proved profitable. All other v21 knobs unchanged.
"""
import logging
...
class FloorBuyV28Strategy(Strategy):
    name = "floor_buy_v28"
    ...
    def __init__(self, params: dict):
        ...
        self.profit_cooldown_h: int = params.get("profit_cooldown_h", 24)
        ...
        self._profit_cooldown_until: dict[int, datetime] = {}
    ...
    def on_tick_batch(
            ...
            if holding > 0:
                ...
                if sell:
                    ...
                    if stopped:
                        ...
                        self._profit_cooldown_until[ea_id] = ts_clean + timedelta(hours=self.profit_cooldown_h)
        ...
        for ea_id, price in ticks:
            ...
            stopped_end = self._stopped_until.get(ea_id, ts_clean)
            profit_end = self._profit_cooldown_until.get(ea_id, ts_clean)
            cooldown_end = max(stopped_end, profit_end)
            if ts_clean < cooldown_end:
                continue
