"""Floor buy v19 — v18 + larger positions (qty 25/18/10).

v18 locked: +$320.9k / 81.2% win / 16 trades / W15 +0.1% / W16 +32%.
The 168h volatility guard successfully excluded ea=252371-class cards.
Now scale size: qty_medium 14 → 18, qty_large 20 → 25, qty_small 8 → 10.

Cash math: at qty=18, each position costs ~$234k. 8 positions × $234k =
$1.87M notional, ~1.9x leveraged against $1M budget via sell-revenue
recycling. Each W16 winner at +30% gross × qty=18 × $13k = +$70k per
trade (vs v18's +$36k at qty=14).
"""
import logging
...
class FloorBuyV19Strategy(Strategy):
    name = "floor_buy_v19"
