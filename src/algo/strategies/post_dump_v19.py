"""Post-dump v19 — halve trigger cooldown to scale v15 via trade count.

Hypothesis: v15's edge is fragile to per-trade sizing bumps (v17 scaled
everything and lost $49k; v18 bumped qty_cap alone and collapsed win
rate from 83% to 38.5%). Sizing alone shifts fills into worse basket
ticks and breaks the edge.

Instead of bumping size, try scaling TRADE COUNT. v15's A-trigger and
B-trigger each enforce an independent 48h cooldown, capping fires to
~3-4 per window. Halving `trigger_cooldown_h` from 48 to 24 roughly
doubles trade opportunities with IDENTICAL per-trade fills — same
basket construction, same qty_cap, same max_positions, same ordering.

If v15's edge is real at micro-size, PnL should scale ~1.5-2x (target
~$220-290k, ~7-10%/wk) with bars holding. If bars break here too,
then more triggers are firing on weaker signal and the architecture
is a dead end for the 25%/wk target — we pivot to combo_v2's pd arm
fix or floor_buy_v21 bar work.

Only ONE param changed vs v15: trigger_cooldown_h 48 -> 24. Every
...
class PostDumpV19Strategy(Strategy):
    name = "post_dump_v19"
