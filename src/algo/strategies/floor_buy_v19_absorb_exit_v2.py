"""Floor buy v19 + absorb-exit overlay v2 — iter 60.

Iter 59 (floor_buy_v19_absorb_exit) over-fired the bearish overlay and
cut v19's winners. Iter 60 adds ONE guard: the overlay only fires when
the position is ALREADY UNDERWATER (current price < buy * 0.97). This
preserves v19's winner-hold behavior completely and tries to cut
confirmed losers before they hit -10% stop.
"""
import logging
...
class FloorBuyV19AbsorbExitV2Strategy(Strategy):
    name = "floor_buy_v19_absorb_exit_v2"
    ...
    def __init__(self, params: dict):
        ...
        # NEW: underwater guard threshold
        self.underwater_threshold: float = params.get("underwater_threshold", 0.97)
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            if holding > 0:
                ...
                # NEW GUARD: only fire overlay if position is already underwater
                is_underwater = buy_price > 0 and price < buy_price * self.underwater_threshold
                if is_underwater and self._bearish_signal(ea_id, ts_clean, price):
