"""Drawdown-reversion v2 — v1 + bottom-confirmation gate (iter 2).

v1 (-$395.6k org, 48.8% win) suffered from buying into continuing
downtrends. W17 (the one winning week, 82%) was entered AFTER capitulation
ended; the losing weeks were entered DURING the fall. A simple momentum
confirmation gate converts "catching knives" into "buying the bounce" —
standard mean-reversion refinement. AUC numbers in the signature report
are survivorship-inflated (they describe dips conditional on reversal),
so we need an additional signal that reversal has begun.

Entry triggers (all v1 triggers retained, using only data available
AT/BEFORE the entry tick):
- drawdown_from_max_72h >= 0.20
- rel_pos_24h <= 0.25
- price_change_72h <= -0.10
- rating in {86..91}
- card_type in v1's premium whitelist
- price in [$13k, $100k]

NEW bottom-confirmation gate (at least ONE must be true):
- price >= min(last_12h) * 1.03                       (bounced 3%+ off 12h low)
- sum of last 6 one-hour price changes >= 0           (positive 6h momentum)

Exit logic, sizing, cooldowns: identical to v1.

Goal: trade signal volume for win rate. v1 had 125 organic trades, 48.8%
win. If we drop to ~50 trades at >=60% win with similar winner size,
PnL turns positive.
"""
import logging
import threading
...
def _load_attrs_sync() -> dict[int, tuple[int, str]]:
    ...
    with _ATTRS_LOCK:
        ...
        if "err" in result_holder:
            logger.error(f"drawdown_reversion_v2: DB load failed: {result_holder['err']}")
            _ATTRS_CACHE = {}
            return _ATTRS_CACHE
...
class DrawdownReversionV2Strategy(Strategy):
    name = "drawdown_reversion_v2"
    ...
    def __init__(self, params: dict):
        ...
        # NEW: bottom-confirmation gate
        self.bounce_window_h: int = params.get("bounce_window_h", 12)
        self.bounce_min_pct: float = params.get("bounce_min_pct", 0.03)
        self.momentum_window_h: int = params.get("momentum_window_h", 6)
        ...
        logger.info(
            f"drawdown_reversion_v2: loaded {len(self._attrs)} players, "
    ...
    def _bottom_confirmed(self, hist_list: list[int], price: int) -> bool:
        """Return True iff EITHER:
          - price >= min of last bounce_window_h prices * (1 + bounce_min_pct)
          - sum of last momentum_window_h one-hour diffs >= 0
        Requires enough history in each sub-window; if a sub-window is
        short, that sub-gate returns False (the other can still pass).
        """
        # Bounce-off-low check. hist_list already includes the current
        # price as its last element (append happened before entry block).
        bounce_ok = False
        if len(hist_list) >= self.bounce_window_h:
            window = hist_list[-self.bounce_window_h:]
            w_min = min(window)
            if w_min > 0 and price >= w_min * (1.0 + self.bounce_min_pct):
                bounce_ok = True

        if bounce_ok:
            return True

        # 6h momentum: sum of last N one-hour price deltas (includes delta
        # into current hour). Needs momentum_window_h+1 points to form N diffs.
        momentum_ok = False
        needed = self.momentum_window_h + 1
        if len(hist_list) >= needed:
            window = hist_list[-needed:]
            diffs = [window[i + 1] - window[i] for i in range(self.momentum_window_h)]
            if sum(diffs) >= 0:
                momentum_ok = True
    ...
    def on_tick_batch(
        ...
        for ea_id, price in ticks:
            ...
            # NEW: bottom-confirmation gate
            if not self._bottom_confirmed(hist_list, price):
                continue
