"""Floor buy v6 — v4 + 72h stability window.

v4 (24h stable, +$49.8k, 56.1% win, corr −0.307): candidate pool still
includes cards that only-just fell to floor. v6 tightens the stability
requirement to 72h: the card must have been ≤floor_stable for three
full days before entry. Cards in active downtrends spend <3 days at any
price level; true floor-proximity cards hover for days.

Everything else matches v4 (ceiling 13k, target 0.30, smoothed stop 0.10,
hard tick stop 0.15, max_hold 96h, qty 8, positions 12).
"""
import logging
...
class FloorBuyV6Strategy(Strategy):
    name = "floor_buy_v6"
