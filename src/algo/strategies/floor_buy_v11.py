"""Floor buy v11 — locked champion from v10 sweep.

48-combo sweep on v10 (profit_target × max_hold × week_max × qty_small ×
qty_large). Best config that passes ≥55% win-rate bar with max PnL:

  profit_target = 0.30, max_hold_h = 96, qty_small = 4, qty_large = 12,
  week_max_ceiling = 18000

Result: +$82,337 / 60.0% win / 40 trades.

Sweep revealed week_max_ceiling 16000 vs 18000 made no difference — the
filter is rarely binding in the ≥5 sph universe (cards that qualify
liquidity-wise rarely exceed $18k in any recent 7d window). Dropping
it as an active filter (kept at 18000 for safety).

Other sweep insights:
  - max_hold 120h gave much bigger PnL (+$146k) but crashed win rate to
    27%. Brief requires ≥55% win; 120h violates it.
  - qty_large 14 over 12 was noisier; qty=12 is the sweet spot.
  - profit_target 0.25 produced similar PnL to 0.30 but higher win rate;
    however 0.30 on 96h max_hold produced the highest PnL over 55% win.

This is the research-doc champion. Absolute PnL below +$100k primary bar
(+$82k is 8.2% over 22 days). W15 likely still negative per v9's pattern.
Honest read: this window is the edge's HARD CEILING on filtered universe.
"""
import logging
...
class FloorBuyV11Strategy(Strategy):
    name = "floor_buy_v11"
