"""Floor buy v22 — v19 with profit_target lowered 0.50 → 0.30.

v19 ORG PnL: W14 $0, W15 −$3.9k, W16 +$349.7k at boundary (100% force-sell).
v21 (max_positions 8→12) proved buy signals fire plenty (+$485k total), but
organic was still only +$11.6k (97.6% boundary share). Signal-bar too high —
lower it so winners exit mid-window.

One-knob change: profit_target 0.50 → 0.30 (smoothed % gain exit trigger).
Keep max_hold_h=240, max_positions=8, qty 10/18/25, all other knobs.
Iter 1 (v20) lesson: do NOT shorten max_hold — that clipped losers AND
winners mid-thesis. Isolate the exit-threshold variable only.
"""
import logging
...
class FloorBuyV22Strategy(Strategy):
    name = "floor_buy_v22"
