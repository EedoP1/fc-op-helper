"""Sweep sell signal params: lookback, threshold, confirmations. No delay."""
...
class HybridPromoDipBuy(PromoDipBuyStrategy):
    """21% strong + snap176 top3, configurable sell signal."""
    ...
    def __init__(self, params: dict):
        ...
        self.sell_lookback: int = params.get("sell_lookback", 12)
        self.sell_confirm_hours: int = params.get("sell_confirm_hours", 1)
    ...
    def on_tick_batch(self, ticks, timestamp, portfolio):
        ...
        for ea_id, price in ticks:
            ...
            if holding > 0:
                ...
                # Sell signal: configurable lookback and threshold
...
async def main():
    ...
    print(f"{'sell_lb':>7} {'sell_thr':>8} {'confirm':>7} {'PnL':>10} {'#':>3} {'W%':>5} {'Sharpe':>7}  {'Mar20':>14} {'Mar27':>14} {'Apr3':>14}  {'Open':>4}")
    print("-" * 115)

    base = {
        "trend_lookback": 12, "min_day": 0, "max_day": 999, "min_crash": 0.05,
        "trailing_stop": 1.0, "max_hold_hours": 336,
        "min_price": 12000, "max_price": 61000, "max_position_pct": 0.10,
        "trend_pct": 0.21,
        "snapshot_top_n": 3, "snapshot_hour": 176, "snapshot_floor": 0.0,
        "stop_delay_hours": 0,  # NO DELAY
    }

    for sell_lb in [12, 24, 36, 48]:
        for sell_thr in [0.05, 0.02, 0.0, -0.02, -0.05, -0.10]:
            for confirm in [1, 3, 6, 12]:
                params = {
                    **base,
                    "sell_lookback": sell_lb,
                    "sell_trend_pct": sell_thr,
                    "sell_confirm_hours": confirm,
                }
                ...
                # Only print interesting results (positive PnL or high win rate)
                if r["total_pnl"] > 100000 or r["win_rate"] >= 0.8:
                    print(f"{sell_lb:>7} {sell_thr:>+8.0%} {confirm:>7} {r['total_pnl']:>+10,} {r['total_trades']:>3} {r['win_rate']:>4.0%} {r['sharpe_ratio']:>7.3f}  {fmt_batch(bp,'03-20'):>14} {fmt_batch(bp,'03-27'):>14} {fmt_batch(bp,'04-03'):>14}  {open_count:>4}")

    # Show all results for best lookbacks
    print(f"\n{'='*115}")
    print("ALL RESULTS")
    print(f"{'='*115}")
    print(f"{'sell_lb':>7} {'sell_thr':>8} {'confirm':>7} {'PnL':>10} {'#':>3} {'W%':>5} {'Sharpe':>7}  {'Mar20':>14} {'Mar27':>14} {'Apr3':>14}  {'Open':>4}")
